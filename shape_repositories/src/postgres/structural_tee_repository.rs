use shapes::aisc_shapes::{ShapeBuilder, ShapeRepository, StructuralTee};
use sqlx::Row;
...
pub struct StructuralTeeRepository {
    pool: Arc<PgPool>,
}

impl StructuralTeeRepository {
    /// Creates a new instance of StructuralTeeRepository type
    /// Takes a pool containing the Postgres database connection
    pub fn new(pool: Arc<PgPool>) -> Self {
        StructuralTeeRepository { pool }
    }
}
...
impl ShapeRepository<StructuralTee> for StructuralTeeRepository {
    ...
    ) -> Pin<Box<dyn Future<Output = Result<Option<StructuralTee>, Box<dyn Error>>> + Send + '_>>
    {
        Box::pin(async move {
            sqlx::query(
                "SELECT 
