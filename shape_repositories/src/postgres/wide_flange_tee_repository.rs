...
impl ShapeRepository<WideFlangeTee> for WideFlangeTeeRepository {
    fn all(
        &self,
    ) -> Pin<Box<dyn Future<Output = Result<Vec<WideFlangeTee>, Box<dyn Error>>> + Send + '_>> {
        Box::pin(async move {
            let rows = sqlx::query(
                "SELECT 
    ...
    ) -> Pin<Box<dyn Future<Output = Result<Option<WideFlangeTee>, Box<dyn Error>>> + Send + '_>> {
        Box::pin(async move {
            sqlx::query(
                "SELECT 
    edi_std_nomenclature,
