use shape_repositories::repositories::{MiscTeeRepository};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let repo = MiscTeeRepository::new(Arc::clone(&conx));
    ...
    let edi_result = repo
        .shape_with_edi_std_nomenclature(String::from("MT6X5"))
        .await;
    match edi_result {
    ...
    let lbl_result = repo
        .shape_with_aisc_manual_label(String::from("MT6X5"))
        .await;
    match lbl_result {
        Ok(shape) => println!("{}", shape.aisc_manual_label),
        Err(err) => println!("{}", err),
    }
    let depth_result = repo.shapes_with_depth(5.0).await;
    match depth_result {
        Ok(shapes) => {
            for shape in shapes {
